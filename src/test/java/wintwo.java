...
//me second method eka
        List<String> list=new ArrayList<String>(handles); //set eka list ekakata convert karanawa
        if(list.size()>1){
            driver.switchTo().window(list.get(1));
            System.out.println("clild tab title is "+driver.getTitle());
            driver.close();
            driver.switchTo().window(oldwindow);
        }
        ...
        WebElement btn2=driver.findElement(By.id("j_idt88:j_idt91"));
        btn2.click();
        Set<String> multiwindowsize=driver.getWindowHandles();
        int count=multiwindowsize.size();
        System.out.println("opened "+count);

        //close all elements expert primary
        //parent window eka witharak open wela anith ewa close karanwa
        WebElement btn3=driver.findElement(By.id("j_idt88:j_idt93"));
        btn3.click();
        Set<String>newwindoeshandle=driver.getWindowHandles();
        for(String element:newwindoeshandle){
            if(!element.equals(oldwindow)){
